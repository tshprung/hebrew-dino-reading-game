...
private const val Ch2Title = "פרק 2 - חוזרים הביתה"

private enum class Ch2PicDino { Idle, Think, Jump }
...
fun Chapter2LevelScreen(
    ...
    when (station) {
        ...
        4 ->
            ...
            val plan =
                StationQuizPlans.chapter2LetterOnly(station)
...
private fun Chapter2PictureStartsWithStation(
    ...
    var dinoState by remember(station) { mutableStateOf(Ch2PicDino.Idle) }
    val dinoTilt = remember { Animatable(0f) }

    val pulse by rememberInfiniteTransition(label = "pulsePicCh2").animateFloat(
        initialValue = 0.98f,
        targetValue = 1.04f,
    ...
    Box(modifier = modifier.fillMaxSize()) {
        ...
        Column(
            ...
            val dinoRes =
                when (dinoState) {
                    Ch2PicDino.Idle -> R.drawable.dino_idle
                    Ch2PicDino.Think -> R.drawable.dino_try_again
                    Ch2PicDino.Jump -> R.drawable.dino_jump_1
                }
            Image(
